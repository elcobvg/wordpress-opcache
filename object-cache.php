...
class WP_Object_Cache
{
    ...
    private function exists($key, $group): bool
    {
        ...
        if (isset($this->cache[$group]) && (isset($this->cache[$group][$key]) || array_key_exists(
                    $key,
                    $this->cache[$group]
                ))) {
            return true;
    ...
    {
        return $this->directory . DIRECTORY_SEPARATOR . $group . DIRECTORY_SEPARATOR . WP_OPCACHE_KEY_SALT . '-' . md5(
                $this->base_name . $key
            ) . '.php';
    }
